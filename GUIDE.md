...
- **JACK2 is built from source** as a `.deb` from the [`TreeFallSound/jack2`](https://github.com/TreeFallSound/jack2) fork (`JACK2_REPO`/`JACK2_TAG` in `config.sh`, pinned by commit). The fork carries the PI-integrator-windup fix, a Python-3.12-capable `waf` backported from 1.9.23, multicast interface pinning, and the netJACK2 master-reaping fixes (see `NETJACK-REAPING.md` in the fork). `debian/patches` now holds only Debian-packaging patches (db 5.3, systemd).
