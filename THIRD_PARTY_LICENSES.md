...
> **Scope:** This file covers the contents of the **pi-gen-pistomp built OS image** released for download — the full system that runs on the device, including the OS, system services (jack2, mod-host, mod-ui, etc.), Python runtime dependencies, and LV2 plugins. This repository is both the build system that produces that image *and* the staging ground for the custom `.deb` packages it ships — the packages built under `debpkgs/` are published from this repo's `gh-pages` apt repository, so their upstream licenses are disclosed here rather than in the downstream `pi-stomp` application repo.

**This project's own build scripts and packaging (this repository) are licensed under AGPL-3.0-or-later**, with the exception of files retained from the upstream `RPi-Distro/pi-gen` project, which remain BSD-3-Clause ([`LICENSE-pi-gen`](./LICENSE-pi-gen)). Components below are run as independent system services, separate processes, or standalone `.deb` packages (communicating via IPC, D-Bus, sockets, or HTTP) rather than linked into this project's own codebase, except where noted.
